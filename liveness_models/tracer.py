...
def get_q_str(state_idx, entity, q_name):
    ret = ""
    for i in range(q_depth):
        mt = readx(state_idx, f"{q_name}_{get_i_name(entity)}[{entity}][{i}].mtype")
        if mt != "Undefined":
            ms = readx(state_idx, f"{q_name}_{get_i_name(entity)}[{entity}][{i}].src").removeprefix(f"OBJSET_")
            if show_acks:
                ae = get_ack_list(state_idx, f"{q_name}_{get_i_name(entity)}[{entity}][{i}].", "acksExpected")
                ret += f"{mt}[{ms}, {ae}], "
            else:
                ret += f"{mt}[{ms}], "
...
def get_state_str(state_idx):
    ...
    for machine in machines:
        ...
        state = readx(state_idx, f"i_{get_i_name(machine)}[{machine}].cb[Address_0].State")
        state = state[state.find("_")+1:]
        ret += f"{machine} = {state}:\n"

        if show_acks:
            ae = get_ack_list(state_idx, f"i_{get_i_name(machine)}[{machine}].cb[Address_0].", "acksExpected")
            ar = get_ack_list(state_idx, f"i_{get_i_name(machine)}[{machine}].cb[Address_0].", "acksReceived")
            if ae:
                ret += "{" f"{ae}, {ar}" + "}\n"
            elif ar:
                ret += "{" f"{ar}" + "}\n"
        ...
        for ch in channels:
            try:
                ret += f"  {ch}: " + get_q_str(state_idx, machine, ch) + "\n"
            except:
                pass
