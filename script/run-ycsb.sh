#!/usr/bin/env bash
#
# run-ycsb.sh - Run YCSB experiments
#
# This script runs YCSB experiments for Figures 13 and 14.
# Uses the configuration file generated by create-conf.sh
#
# Usage:
#   ./script/run-ycsb.sh                  # Run all YCSB workloads
#   ./script/run-ycsb.sh workloada        # Run only workload A
#   ./script/run-ycsb.sh workloadb        # Run only workload B
#
# Required for: Figure 13, 14
#
set -eo pipefail
...
# Check if YCSB is built
if [[ ! -f "${REPO_ROOT}/benchmarks/YCSB-C/ycsbc" ]]; then
    echo "ERROR: YCSB benchmark not built."
...
PID_FILE="${DATA_DIR}/running_pids_ycsb.txt"
> "${PID_FILE}"
...
echo "Required for: Figures 13, 14"
echo ""
echo "Data directory: ${DATA_DIR}"
echo "Log directory: ${LOG_DIR}"
[[ -n "${FILTER_WORKLOAD}" ]] && echo "Filter workload: ${FILTER_WORKLOAD}"
[[ -n "${FILTER_PROTOCOL}" ]] && echo "Filter protocol: ${FILTER_PROTOCOL}"
echo "=============================================="
echo ""
echo "YCSB Workloads:"
echo "  A - 50% Read, 50% Update"
echo "  B - 95% Read, 5% Update"
echo "  C - 100% Read"
echo "  D - 95% Read, 5% Insert"
echo "  F - 50% Read, 50% Read-Modify-Write"
echo ""
...
while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    # Apply filters
    [[ -n "${FILTER_WORKLOAD}" && ! "$name" =~ ${FILTER_WORKLOAD} ]] && continue
...
echo ""
echo "=============================================="
echo "Started ${experiments_started} YCSB experiments"
echo "=============================================="
echo ""

if [[ ${#running_pids[@]} -gt 0 ]]; then
    echo "Waiting for experiments to complete..."
    echo "Monitor progress: tail -f ${LOG_DIR}/ycsb_*.log"
    echo ""
    ...
    echo ""
    echo "=============================================="
    echo "All YCSB experiments completed!"
    echo "=============================================="
fi

echo ""
echo "Results saved in: ${DATA_DIR}/gem5.output/ycsb/"
echo "Logs saved in: ${LOG_DIR}/"
echo ""
echo "These results are required for: Figures 13, 14"
echo ""
